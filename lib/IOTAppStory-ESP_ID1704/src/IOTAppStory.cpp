...
bool IOTAppStory::isConnected(){
	return _connected;
}

void IOTAppStory::configureNewNetwork(){
	{
		...
		if(CFG_INCLUDE){ 
			{
				...
				// run config server
				configServer configServer(*this, config);
				configServer.runSimpleConnect();    //RUNS config server that does not need to manually exit config mode
			#ifdef ESP8266
			}	
			#endif
			delay(100);
